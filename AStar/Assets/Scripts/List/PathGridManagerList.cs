...
namespace AStarList
{
    public class PathGridManagerList : MonoBehaviour
    {
        public Transform _player,
            _target;
        public LayerMask _obstacleMask;
        public float _halfNodeWidth;
        public NodeList[,] _grid;
        ...
        private void Start()
        {
            ...
            _grid = new NodeList[GridWidth, GridHeight];
            ...
            for (int i = 0; i < GridHeight; i++)
            {
                for (int j = 0; j < GridWidth; j++)
                {
                    ...
                    if (Physics.CheckSphere(_grid[i, j].m_vPosition,
                        ...
                        _obstacleMask))
                    {
                        _grid[i, j]._nodeType = NodeList.NodeType.blocked;
                    }
                    else
                    {
                        _grid[i, j]._nodeType = NodeList.NodeType.open;
                    }
                }
            }
        }

        private List<NodeList> ReturnNeighbours(PathGridManagerList pathGridManager, NodeList _currentNode)
        {
            List<NodeList> _neighbourList = new List<NodeList>();
        ...
        {
            foreach (NodeList node in _grid)
            {
        ...
        {
            NodeList playerGridCoordinates = GridLocator(_player.position);
            NodeList targetGridCoordinates = GridLocator(_target.position);

            for (int i = 0; i < GridHeight; i++)
            {
                for (int j = 0; j < GridWidth; j++)
                {
                    if (_grid[i, j]._nodeType == NodeList.NodeType.open)
                    {
                        Gizmos.color = Color.white;
                    }

                    if (_grid[i, j]._nodeType == NodeList.NodeType.blocked)
                    {
        ...
        public NodeList GridLocator(Vector3 location)
        {
